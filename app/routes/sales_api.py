...
Provides endpoints for leads, customers, sales statistics, and profile management
"""
...
from app.database.users import UserInDB, DatabaseUsers, UserUpdate
...
async def get_profile(current_user: UserInDB = Depends(get_current_user)):
    ...
    if not is_sales_user(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Sales department access required"
        )
...
@router.post("/profile/documents/upload")
...
@router.get("/profile/documents/categories")
...
async def download_document(
    ...
    # Handle authentication via token parameter or header
    if token and not current_user:
        try:
            # Verify token from query parameter using JWT
...
@router.get("/profile/documents/download-all")
...
@router.get("/profile/salary-slips")
async def get_salary_slips(current_user: UserInDB = Depends(get_current_user)):
    ...
    try:
        print(f"Getting salary slips for sales user: {current_user.id}")
...
@router.get("/profile/salary-slips/{slip_id}/download")
async def download_salary_slip(
    ...
    try:
        print(f"Sales download request for slip_id: {slip_id}, user: {current_user.id}")
...
@router.get("/profile/salary-slips/current")
async def download_current_salary_slip(
    ...
    try:
        print(f"Sales current salary slip requested by user: {current_user.id}")
        now = get_ist_now()
...
@router.get("/profile/postal-code/{pincode}")
async def get_postal_info(pincode: str, current_user: UserInDB = Depends(get_current_user)):
...
@router.get("/profile/states")
async def get_indian_states(current_user: UserInDB = Depends(get_current_user)):
