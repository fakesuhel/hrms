...
window.RoleManager = {
    ...
    // Initialize method for dashboard compatibility
    initialize: async function() {
        console.log('RoleManager initialized');
        try {
            // Get current user and update sidebar
            const user = await RoleAwareAPI.getCurrentUser();
            if (user) {
                window.currentUser = user;
                this.updateSidebarNavigation(user);
                console.log('Sidebar navigation updated for user:', user.username);
            }
        } catch (error) {
    ...
    // Filter page content method for dashboard compatibility
    filterPageContent: function(pageType) {
        console.log(`Filtering page content for: ${pageType}`);
        // This can be expanded based on page-specific filtering needs
    }
};
